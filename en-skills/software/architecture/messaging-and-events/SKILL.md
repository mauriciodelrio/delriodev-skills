...
description: >
  Decision tree for inter-service communication and async processing.
  Covers SQS, SNS, EventBridge, WebSockets, and event-driven patterns. Includes
  criteria by communication type, volume, delivery guarantees, and cost.
  Do not assume — evaluate whether the project really needs messaging.
---
...
Types:
  - Standard:  At-least-once delivery, order not guaranteed
               → Most use cases
  - FIFO:      Exactly-once, guaranteed order
               → When order matters (payments, sequences)
...
Cost:
  ...
  - Archive: $0.023/GB stored
  - Scheduler: free (serverless invocations)
```
...
```
MONOLITH (most projects start here):
  → You don't need messaging
  → Async functions: Simple job queue with BullMQ + Redis
  → When you feel real pain (coupling, scalability), then migrate

MODULAR MONOLITH:
  → EventBridge for communication between modules
  → SQS for heavy async work
  → Keep the shared DB but communicate via events

MICROSERVICES (only if scale demands it):
  → EventBridge as backbone
  → SQS for each consumer
